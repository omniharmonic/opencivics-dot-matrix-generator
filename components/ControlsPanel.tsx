...
export const ControlsPanel: React.FC<ControlsPanelProps> = ({
  ...
  return (
    <div className="bg-gray-800 text-white p-6 rounded-lg shadow-2xl w-full max-w-sm flex flex-col space-y-4 max-h-[90vh] overflow-y-auto">
      <h2 className="text-2xl font-bold text-center border-b border-gray-700 pb-3">Generative Controls</h2>
      
      {/* Grid Structure */}
      <div className="space-y-4 pt-2">
        <h3 className="text-lg font-semibold text-teal-400">Grid Structure</h3>
      ...
      <div className="space-y-4 pt-2 border-t border-gray-700">
        <h3 className="text-lg font-semibold text-teal-400">Connection Rules</h3>
        <div>
        ...
        <Slider label="Connection Start Ring" name="startRing" value={settings.startRing.toString()} current={settings.startRing} min={0} max={maxRing} step={1} onChange={handleInputChange} />
        <Slider label="Connection End Ring" name="endRing" value={settings.endRing.toString()} current={settings.endRing} min={0} max={maxRing} step={1} onChange={handleInputChange} />
      </div>
      
      {/* Algorithm Parameters */}
      <div className="space-y-4 pt-2 border-t border-gray-700">
         <h3 className="text-lg font-semibold text-teal-400">Algorithm Parameters</h3>
         ...
         {settings.strategy === Strategy.Random && <Slider label="Max Connections/Point" name="maxConnections" value={settings.maxConnections.toString()} current={settings.maxConnections} min={1} max={5} step={1} onChange={handleInputChange} />}
      </div>

      {/* Appearance */}
      <div className="space-y-4 pt-2 border-t border-gray-700">
        <h3 className="text-lg font-semibold text-teal-400">Appearance</h3>
      ...
      <div className="pt-2 border-t border-gray-700">
        <div className="flex items-center space-x-2">
          <label htmlFor="seed" className="block text-sm font-medium text-gray-400">Seed: {settings.seed}</label>
          <button onClick={onRandomizeSeed} className="flex-grow bg-teal-500 hover:bg-teal-600 text-white font-bold py-2 px-2 rounded-md transition duration-300">
            Randomize
          </button>
        </div>
      </div>
      ...
      <div className="flex space-x-2 pt-2 border-t border-gray-700">
        <button onClick={onExportSvg} className="flex-1 bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded-md transition duration-300">
          Export SVG
        </button>
        <button onClick={onExportPng} className="flex-1 bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded-md transition duration-300">
          Export PNG
        </button>
      </div>
